	...
	class Mo_Ldap_Login{
		...
		function login_widget_save_options(){
			if(isset($_POST['option'])){
				...
				} else if($_POST['option'] == 'mo_ldap_ping_server'){
					
					//Sanitize form fields
					$ldap_server_url = sanitize_text_field($_POST['ldap_server']);
					$customer = new Mo_Ldap_Customer();
					$content = json_decode($customer->ping_ldap_server($ldap_server_url), true);
					
					if(strcasecmp($content['statusCode'], 'SUCCESS') == 0){
						update_option('mo_ldap_message', 'Successfully contacted LDAP Server');
						$this->show_success_message();
					} else{
						$error_message = $content['statusMessage'];
						update_option('mo_ldap_message', 'Error contacting LDAP Server: ' . $error_message);
						$this->show_error_message();
					}
